...
test("express template to strings", () => {
    ...
    const template = parseSource("<h1 name={$scope.gifts[0]}>{$scope.gifts[1]} {$scope.gifts[2]}</h1>", scope);
...
test("express attribute", () => {
    ...
    expect(expressAttributeFirstValue(template.children[0] as HtmlElement, "gifts")).toBe(scope.gifts);
});
...
test("express defined attributes to strings", () => {
    ...
    const template = parseSource(`<hr gift="joy" chapter={5} both="{$scope.gift} {$scope.chapter}">`);
    template.scope.gift = "joy";
    template.scope.chapter = 5;
    expect(expressDefinedAttributesToStrings(template.children[0] as HtmlElement, types)).toEqual({
        gift: "joy",
        chapter: "5",
        both: "joy 5",
    });
